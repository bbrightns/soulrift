...
const SPELLS_DATA = [
  ...
  { // Burn
    ...
    price: 150,
    obtain: 'shop',
    desc: 'Apply Burn: DoT lasting (spell level) turns, stacks build for Explosion Burn.',
    scaling: '',
    condition: 'None. Stack 3–4 times before using Explosion Burn.',
    effect: 'Deal minor direct fire damage. Push 1 DoT entry that ticks (basePower × 0.35 × lvlMult) damage per turn for (spellLvl) turns. Each cast also increments the permanent Burn stack counter used by Explosion Burn and Ragnarok Ignition. Stack counter never resets until consumed.',
  ...
  { // Fire Thief
    ...
    price: 180,
    obtain: 'shop',
    desc: 'Deal fire damage and steal Gold — doubled if any Burn DoT is active.',
    scaling: '',
    condition: 'Apply Burn first to double Gold steal per cast.',
    effect: 'Deal fire damage (basePower + STR scaling). Steal (12 + 3×(lvl-1)) to (21 + 3×(lvl-1)) Gold. If any Burn DoT entry is currently active, steal amount doubles. Gold is added immediately.',
  ...
  { // Ember Skin
    ...
    price: 150,
    obtain: 'shop',
    desc: 'Reduce incoming direct damage by 25% for (2 + lvl-1) turns.',
    scaling: '',
    condition: 'Best used Turn 1 to survive the monster-first opening.',
    effect: 'Apply Ember Skin buff for (2 + spellLvl - 1) turns. Each incoming direct enemy attack is reduced by 25% before it hits HP. DoT and passive damage bypasses this reduction. Higher level extends duration, not reduction strength.',
  ...
  { // Fire Storm
    ...
    price: 0,
    obtain: 'drop',
    desc: 'Sustained fire DoT over 2 turns. Disruption mechanic planned for PvP.',
    scaling: '',
    condition: 'Best against Light Tower enemies who rely on Charge Release.',
    effect: 'Deal fire damage as DoT: push 1 entry dealing (basePower × 0.55) per turn for 2 turns. Does not add to Burn stack counter. Note: charge-type spell disruption (40% chance) is reserved for future PvP implementation.',
